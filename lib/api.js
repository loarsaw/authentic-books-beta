...
export async function getMergeRequestPosts(merge_id) {
  const merge_api_url = `${COSMIC_API_URL}/v1/${BUCKET_SLUG}/merge-requests/${merge_id}/objects?read_key=${READ_KEY}&pretty=true&props=slug,title,content,metadata,created_at,type_slug`;
  const data = await fetch(merge_api_url).then((response) => response.json());
  return data.objects;
}
